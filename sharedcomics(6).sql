...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-06-2026 a las 19:35:21
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminar_comic` (IN `p_comic_id` INT)   BEGIN
    -- 1. Eliminar el registro del cómic
    -- (Las tablas 'paginas' y 'comic_categoria' se limpian solas por el ON DELETE CASCADE)
    DELETE FROM `comics` WHERE `id` = p_comic_id;
END$$

--
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `obtener_total_comics_usuario` (`p_usuario_id` INT) RETURNS INT(11) READS SQL DATA BEGIN
    DECLARE v_total INT DEFAULT 0;
    
    -- Contar los cómics asignados al ID del usuario
...
--
-- Volcado de datos para la tabla `comic_categoria`
--

INSERT INTO `comic_categoria` (`comic_id`, `categoria_id`) VALUES
(1, 1),
(2, 1),
(33, 2),
(34, 2),
