<?php
// 1. Inicio de sesión y conexión a la base de datos (Igual que en index.php)
session_start();
...
// Filtro por Autor / Usuario que lo subió
if ($autor_id !== '') {
    $autor_id_limpio = $autor_id;
    $query .= " AND usuario_id = $autor_id_limpio";
}

// Ordenar por fecha de subida de la misma manera que las novedades
...
<body class="bg-light">
    ...
    <main class="container my-5">
        ...
        <div class="card p-4 border border border-3 rounded shadow-sm bg-white mb-5">
            <h2 class="text-center mb-4 fw-bold text-uppercase" style="letter-spacing: 1px;">
                Buscador de Cómics
            </h2>
            ...
            <form action="buscador.php" method="GET">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2 d-flex align-items-end">
                        <button type="submit" class="btn btn-dark w-100 fw-bold py-2 text-uppercase">
                            Buscar
                        </button>
                    </div>
                </div>
            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-5 g-4 justify-content-center">
                ...
                if ($resultado_comics && $resultado_comics->num_rows > 0) {
                    while ($comic = $resultado_comics->fetch_assoc()) {
                        // Usamos el ID correcto que entrega la vista
                                ...
                                <div class="card-body d-flex flex-column p-3">
                                    ...
                                    <div class="mb-3">
                                        <a class="small text-decoration-none fw-semibold d-inline-block mb-1"
                                            ...
                                            aria-controls="desc-<?php echo $id_comic; ?>">
                                            📄 Leer descripción...
                                        </a>

                                        <div class="collapse" id="desc-<?php echo $id_comic; ?>">
                                            <div class="card card-body text-white p-2 border-0 mt-1 small"
                                                style="font-size: 0.8rem; line-height: 1.4;">
                                                <?php echo ($comic['comic_descripcion']); ?>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <a href="comic.php?id=<?php echo $id_comic; ?>"
                                        class="btn btn-sm btn-outline-dark fw-bold mt-auto w-100">
                                        Ver Comic
                                    </a>
                                </div>
                    ...
                    // Alerta si no coincide ningún registro
                    echo "
